...
findall(X,no(_,X,_,_,_,_,_,_,_,_,_,_,_),Lista),
ListaAux = [],
...
comparar1(_,[],[],_):-!.
comparar1(Comb,[Id|Lista],ListaFinal,_):- no(_,Id,ListaTags,_,_,_,_,_,_,_,_,_,_),intersection(Comb,ListaTags,LInter),length(LInter,TamFinal),length(Comb,TamComb),(TamFinal \== TamComb),!,comparar1(Comb,Lista,ListaFinal,_).
