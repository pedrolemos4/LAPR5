lista_utilizadores_com_tags_conexoes(Utilizador,Nivel,ListaResTags,ListaNUtilizadores,ListaFinal1):- no(NUtilizador,Utilizador,ListaTags),
...
lista_todos_utilizadores(Utilizador,ListaTodos):- findall(X, (no(_,X,_)), List) ,delete(List,Utilizador,ListaTodos).
...
lista_utilizadores_sem_ligacao(_,ListaUtilizadores,[],ListaFinal):-!, reverse(ListaUtilizadores,ListaFinal).
lista_utilizadores_sem_ligacao(NUtilizador,ListaUtilizadores,[NX|ListaTodos],ListaFinal):- ((((ligacao(NUtilizador,NX,_,_));(ligacao(NX,NUtilizador,_,_))), lista_utilizadores_sem_ligacao(NUtilizador,ListaUtilizadores,ListaTodos,ListaFinal)); lista_utilizadores_sem_ligacao(NUtilizador,[NX|ListaUtilizadores],ListaTodos,ListaFinal)).
...
dfs2(NAct,Dest,LA,Cam):-(ligacao(NAct,NX,_,_);ligacao(NX,NAct,_,_)),\+ member(NX,LA),dfs2(NX,Dest,[NX|LA],Cam).
